...
#. Server: select an "identity" URL from :ref:`apis` and enter only the server
...
To be able to configure WinSCP you need to have EC2 credentials for the
Catalyst Cloud account you intend to use. You can obtain your EC2 credentials
from the dashboard (under Access & Security, API Access), or by using the
command line:
...
#. Host name: select an appropriate "s3" endpoint from
   :ref:`the API page <apis>`.
#. Access key ID & Secret access key: The EC2 credentials from above
